...
class GateFormer(TwoTowerBaseModel):
    ...
    def __init__(self, manager, embedding, encoderN, encoderU, reducer, aggregator=None):
        ...
        if aggregator is not None:
            self.name = '__'.join(['tesrec', manager.bert, manager.encoderN, manager.encoderU, manager.reducer, manager.aggregator, str(manager.k)])
        else:
            self.name = '__'.join(['tesrec', manager.bert, manager.encoderN, manager.encoderU, manager.reducer, str(manager.k), manager.verbose])
