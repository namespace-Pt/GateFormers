...
from utils.util import _group_lists
...
class Manager():
    ...
    def __init__(self, config=None, notebook=False):
        ...
        parser.add_argument("-sav", "--save-at-validate", help="save at each validation", action="store_true", default=False)
        ...
        parser.add_argument("-lr", dest="learning_rate", help="learning rate of non-bert modules", type=float, default=1e-5)
        parser.add_argument("-blr", "--bert_lr", dest="bert_learning_rate", help="learning rate of bert based modules", type=float, default=1e-5)
        ...
        parser.add_argument("-ws", "--world_size", dest="world_size", help="total number of gpus", default=1, type=int)
        ...
        self.exclude_hparams = ["exclude_hparams", "epochs", "device", "path", "title_length", "step", "checkpoint", "smoothing", "num_workers", "pin_memory", "interval", "npratio", "metrics", "aggregator", "head_num", "rank", "unilm_path", "unilm_config_path"]


    def __str__(self):
        return str({k:v for k,v in vars(self).items() if k not in self.exclude_hparams})
    ...
    def prepare(self):
        ...
        """
        from .dataset import MIND, MIND_news, MIND_history
        from .util import Partition_Sampler
        ...
        if self.mode == "train":
            ...
            loader_train = DataLoader(dataset_train, batch_size=self.batch_size,
                                    drop_last=False, shuffle=shuffle, sampler=sampler_train)
            loader_dev = DataLoader(dataset_dev, batch_size=1,
                                    drop_last=False, shuffle=False, sampler=sampler_dev)

            if self.fast:
                dataset_news = MIND_news(self, file_directory_dev)
                loader_news = DataLoader(dataset_news, batch_size=self.batch_size_news,
                        drop_last=False)
                ...
                loader_dev = DataLoader(dataset_dev, batch_size=1,
                                        drop_last=False, sampler=sampler_dev)
            ...
            loader_test = DataLoader(dataset_test, batch_size=1,
                        drop_last=False, sampler=sampler_test)
            ...
            from .dataset import MIND_recall
            from .util import collate_recall
            file_directory = self.path + "MIND/MINDlarge_dev/"

            dataset = MIND_recall(self, file_directory)
            loader = DataLoader(dataset, batch_size=self.batch_size_news,
                                    drop_last=False, collate_fn=collate_recall)

            return loader,


    def save(self, model, step, optimizer=None, scheduler=None, best=False):
        """
            shortcut for saving the model and optimizer
        """
        if best:
            save_path = "data/ckpts/{}/best.model".format(model.module.name if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.name)
        else:
            save_path = "data/ckpts/{}/step{}.model".format(model.module.name if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.name, step)
    ...
    def load(self, model, checkpoint, optimizer=None, scheduler=None, strict=True):
        """
        ...
        if checkpoint == 0:
            ...
            save_path = "data/ckpts/{}/step{}.model".format(model.name, checkpoint)
        ...
        with open("performance.log", "a+") as f:
            ...
            if not self.no_email:
                try:
                    ...
                    subject = "[PR] {} : {}".format(d["name"], res["auc"])
    ...
    def _get_optimizer(self, model, loader_train_length):
        """
        ...
        if self.scheduler == "linear":
            ...
            scheduler = get_linear_schedule_with_warmup(optimizer,
                                            num_warmup_steps = round(self.warmup * total_steps),
    ...
    def _eval_fast(self, model, loaders):
        ...
        if self.rank in [-1, 0]:
            cache_directory = "data/cache/tensors/{}/{}/dev/".format(model.name, self.scale)
    ...
    def evaluate(self, model, loaders, load=False, log=True, optimizer=None, scheduler=None):
        ...
        if self.rank in [0, -1]:
            ...
            logger.info("\nevaluation result of {} is {}".format(model.module.name if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.name, res))

            if log and self.rank in [0, -1]:
                res["step"] = self.checkpoint
                self._log(res, model.module.name if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.name)

        return res


    def _train(self, model, loaders, optimizer, loss_func, validate_step, hold_step, scheduler=None, save_at_validate=False):
        ...
        for epoch in range(self.epochs):
            ...
            tqdm_ = tqdm(loaders[0], ncols=120)
            ...
            for step, x in enumerate(tqdm_, 1):
                ...
                if total_steps > hold_step and total_steps % validate_step == 0:
                    ...
                    if self.rank == 0:
                        ...
                        if save_at_validate:
                            self.save(model, total_steps, optimizer)

                        if result["auc"] > best_res["auc"]:
                            best_res = result
                            self.save(model, total_steps, optimizer, best=True)
    ...
    def train(self, model, loaders):
        ...
        optimizer, scheduler = self._get_optimizer(model, len(loaders[0]))
        ...
        if self.validate_step[-1] == "e":
            ...
            validate_step = len(loaders[0]) * int(self.validate_step[:-1])
        elif self.validate_step == "0":
            # validate at the end of every epoch
            validate_step = len(loaders[0])
        else:
            # validate at certain steps
            validate_step = int(self.validate_step)
        if self.hold_step[-1] == "e":
            hold_step = len(loaders[0]) * int(self.hold_step[:-1])
        else:
            hold_step = int(self.hold_step)

        if self.anomaly:
            with torch.autograd.set_detect_anomaly(True):
                res = self._train(model, loaders, optimizer, loss_func, scheduler=scheduler, validate_step=validate_step, hold_step=hold_step, save_at_validate=self.save_at_validate)
        else:
            res = self._train(model, loaders, optimizer, loss_func, scheduler=scheduler, validate_step=validate_step, hold_step=hold_step, save_at_validate=self.save_at_validate)

        if self.rank in [-1,0]:
            logger.info("Best result: {}".format(res))
            self._log(model.module.name if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.name, res)
    ...
    def _test_fast(self, model, loaders):
        ...
        if self.rank in [-1, 0]:
            cache_directory = "data/cache/tensors/{}/{}/test/".format(model.name, self.scale)
            os.makedirs(cache_directory, exist_ok=True)
    ...
    def test(self, model, loaders):
        ...
        if self.rank in [0, -1]:
            save_directory = "data/results/{}".format(model.module.name if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.name + "/{}_step{}".format(self.scale, self.checkpoint))
    ...
    def inspect(self, model, loaders):
        ...
        model.eval()
        logger.info("inspecting {}...".format(model.module.name if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.name))
    ...
    def recall(self, model, loaders):
        ...
        if self.recall_type == "d":
            ...
            print(len(news_set))
            try:
                news_reprs = torch.load("data/cache/tensors/{}/large/dev/news.pt".format(model.module.name if isinstance(model, torch.nn.parallel.DistributedDataParallel) else model.name), map_location=device)
            except:
            ...
            try:
                ...
                from .util import BM25_token, construct_inverted_index
